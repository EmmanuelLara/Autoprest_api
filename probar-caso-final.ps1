# Script final para probar el caso específico del usuario

Write-Host "🎯 Probando el caso específico del usuario..." -ForegroundColor Green
...
Write-Host ""
Write-Host "🎯 Probando el caso específico..." -ForegroundColor Yellow
...
$testTokenValido = Test-Endpoint-Custom -Url "http://localhost:3000/gerente/vehiculos" -Method "GET" -Description "Probar con token válido" -CustomHeaders $headersValidos
...
$testTokenConLetraExtra = Test-Endpoint-Custom -Url "http://localhost:3000/gerente/vehiculos" -Method "GET" -Description "Probar con token con letra extra" -CustomHeaders $headersTokenConLetraExtra
...
$testTokenConLetraEnMedio = Test-Endpoint-Custom -Url "http://localhost:3000/gerente/vehiculos" -Method "GET" -Description "Probar con token con letra en medio" -CustomHeaders $headersTokenConLetraEnMedio
...
$testTokenAleatorio = Test-Endpoint-Custom -Url "http://localhost:3000/gerente/vehiculos" -Method "GET" -Description "Probar con token aleatorio" -CustomHeaders $headersTokenAleatorio

Write-Host ""
Write-Host "🎉 ¡Pruebas del caso específico completadas!" -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "🔗 URL de la API: http://localhost:3000" -ForegroundColor Cyan
Write-Host "📚 Documentación: http://localhost:3000/api-docs/" -ForegroundColor Cyan
Write-Host ""
Write-Host "💡 Si las pruebas 2, 3 y 4 muestran 'OK' en lugar de errores, hay un problema con la validación" -ForegroundColor Red
Write-Host "💡 Si muestran errores 403, la validación está funcionando correctamente" -ForegroundColor Green
